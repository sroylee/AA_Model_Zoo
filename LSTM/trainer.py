...
class Trainer():
    ...
    def test(self, load_para, batch_size=1):
        _, test_text, _, test_label, _, test_len = self.data_loader.data_spliter()
        #load parameters
        if load_para:
        ...
        if batch_num*batch_size < len(test_text):
            batch_num += 1
        correct = 0
        ...
        for batch_i in range(batch_num):
            ...
            batch_test_len = test_len[batch_start: batch_end]
            
            #sort
            batch_test_text = [x for (y,x) in sorted(zip(batch_test_len,batch_test_text), reverse=True)]
            batch_test_label = [x for (y,x) in sorted(zip(batch_test_len,batch_test_label), reverse=True)]
            batch_test_len = sorted(batch_test_len, reverse= True)
            ...
            predict = self.cnn_n(batch_test_text_tensor, batch_test_len)
    ...
    def train(self):
        ...
        for epoch in range(1, self.epoch+1):
            self.cnn_n.train()
            start = time.time()
            _, _, train_label, test_label, _, _ = self.data_loader.data_spliter()
            ...
            for batch_elem in train_generator:
                ...
                batch_train_len = batch_elem['batch_train_len']
                #pbar.update(1)
                ...
                predict = self.cnn_n(batch_train_text_tensor, batch_train_len)
                predict_class = torch.argmax(F.softmax(predict, dim=1), dim=1).cpu().numpy()
...
def main():
    ...
    for i in range(1,2):
        ...
        cnn_n = EncoderRNN(args, data_info['emb_num']).cuda()
