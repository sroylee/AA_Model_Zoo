        ...
        for file in file_list:
            ...
            with open(file_path, 'r') as f:
                ...
                for line in f:
                    ...
                    if self.mode == 'word':
                        line[0] = line[0].split(' ')
                    elif self.mode[:-1] == 'char':
                        #line[0] = ''.join(line[0].split(' '))
                        line[0] = analyzer(line[0])
                    #print(bigram_text)
                    for bigram in line[0]:
                        if bigram not in emb_list:
                            emb_list.add(bigram)
...
#                             continue
        ...
        print('data processing done, cost%f h'%((end-start)/3600))
...
class Dataloader():
    ...
    def data_spliter(self):
        ...
        return np.array(train_text), np.array(test_text), np.array(train_label), np.array(test_label)
    
    def data_iter_train(self):
        batch_size=self.batch_size
        train_text, test_text, train_label, test_label = self.data_spliter()
        data_temp = list(zip(train_text, train_label))
        random.seed(10)
        random.shuffle(data_temp)
        train_text, train_label = zip(*data_temp)
