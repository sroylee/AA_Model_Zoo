...
def average_word_length(text_doc):
    word_list = text_doc.split(" ")       #decode('utf-8').split(" ")
...
def count_letter_freq(text_doc, vocab):  # per document (vector with length 100)
#     text_doc = ''.join([i for i in text_doc if i.isalpha()])
    text_doc = ''.join(text_doc.split(' '))
    letter = vocab #['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's'
              #, 't', 'u', 'v', 'w', 'x', 'y', 'z']
    count = {}
    for s in text_doc:
        #if count.has_key(s):
    ...
    for d in letter:
        if d in count.keys():
            ...
            count_list[d] = 0
    assert len(text_doc)!=0, 'length of text_doc equal to 0!'
...
def freq_function_word(text):  # per document (vector with length 746)
    #words = text.decode('utf-8').split(" ")
    words = text.split(" ")
    fn = os.path.join(os.path.dirname(__file__), "stopwords_zh.txt")
    with open(fn) as f:
...
def pos_tag_freq(text):
    ...
    for tag in pos_tag:
        ...
        if freq is None:
            ...
            count_tag[tag] = freq
    return np.array(list(count_tag.values()))/len(word_list)
...
def vocab_gen(text, x_train):
    vec = CountVectorizer(analyzer="char", ngram_range=(1, 1), max_df=0.95, min_df=2, max_features=100)
    vec.fit_transform(x_train)
    vocab = vec.vocabulary_
    return vocab
...
def create_feature(text_x, x_train, mode):
    ...
    if mode == 0:
        ...
        vocab = vocab_gen(text_x, x_train)
        ...
        for x in text_x:
            ...
            stylometry_vector.extend(count_letter_freq(x, vocab))
